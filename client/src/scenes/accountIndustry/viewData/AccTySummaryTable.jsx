...
function AccTySummaryTable({ data = [] }) {
  ...
  const zebra = (idx) =>
    alpha(
      ...
      theme.palette.mode === "light" ? 0.25 : 0.15
    );
  ...
  const custTypes = [...new Set(data.map((d) => d.Acct_Industry))];
  const summary = {};
  quarters.forEach((q) => {
    summary[q] = {};
          ...
          <TableRow>
            {[...quarters, "Total"].map((key) => (
              <React.Fragment key={key}>
                <TableCell
                  align="right"
                  sx={{ backgroundColor: subHeaderBg, fontWeight: "bold" }}
                >
                  # of Opps
                </TableCell>
                <TableCell
          ...
          {custTypes.map((type, idx) => (
            ...
            >
              ...
              <TableCell align="right">
                {formatK(typeTotals[type].acv)}
              </TableCell>
              <TableCell align="right">
                {grandTotal.acv
                  ? `${Math.round(
                      (typeTotals[type].acv / grandTotal.acv) * 100
                    )}%`
                  : "0%"}
              </TableCell>
            </TableRow>
...
export default AccTySummaryTable;
