...
function TeamStackedBarChart({ data, width = 600, height = 400 }) {
    const ref = useRef();
    ...
    useEffect(() => {
        ...
        const types = [...new Set(data.map(d => d.Team))];

        const dataByQuarter = quarters.map(q => {
            const entry = { quarter: q };
            let total = 0;
            types.forEach(type => {
                const val = data.find(d => d.closed_fiscal_quarter === q && d.Team === type)?.acv || 0;
                entry[type] = val;
                total += val;
            });
            entry.total = total;
...
export default TeamStackedBarChart;
