...
function AcvRStackedBarChart({ data, width = 600, height = 400 }) {
    const ref = useRef();
    ...
    useEffect(() => {
        ...
        const types = [...new Set(data.map(d => d.ACV_Range))];

        const dataByQuarter = quarters.map(q => {
            const entry = { quarter: q };
            let total = 0;
            types.forEach(type => {
                const val = data.find(d => d.closed_fiscal_quarter === q && d.ACV_Range === type)?.acv || 0;
                entry[type] = val;
                total += val;
            });
            entry.total = total;
        ...
        const layerG = g.selectAll(".layer")
            ...
            .join("g")
            .attr("class", "layer")
            .attr("fill", d => COLORS[d.key] || d3.schemeCategory10[types.indexOf(d.key) % 10]);
...
export default AcvRStackedBarChart;
