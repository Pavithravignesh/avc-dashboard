...
function CusTySummaryTable({ data = [] }) {
    ...
    const custTypes = [...new Set(data.map(d => d.Cust_Type))];
    const summary = {};
    quarters.forEach(q => {
        summary[q] = {};
        custTypes.forEach(type => {
            const rows = data.filter(d => d.closed_fiscal_quarter === q && d.Cust_Type === type);
            summary[q][type] = {
...
export default CusTySummaryTable;
